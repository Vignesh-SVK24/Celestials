  ...
  return (
    <section id="team" className="py-24 md:py-32 bg-black text-white relative overflow-hidden min-h-screen flex flex-col justify-center">
      ...
      <div className="relative w-full max-w-6xl mx-auto h-[50vh] md:h-[60vh] flex items-center justify-center perspective-[1000px]">
        ...
        <div className="relative z-10 w-[82vw] max-w-xs md:w-1/3 h-[420px] md:h-full cursor-pointer group" onClick={() => onMemberSelect(activeMember)}>
          <AnimatePresence initial={false} custom={direction}>
            ...
            >
              <div className="w-full h-full rounded-[60px] md:rounded-[100px] overflow-hidden relative border border-white/10 group-hover:border-white/30 transition-colors shadow-2xl">
                ...
                <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 flex items-center justify-center opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="px-5 py-2 bg-black/60 md:bg-white/10 backdrop-blur-md rounded-full text-[11px] md:text-xs tracking-widest uppercase border border-white/20 text-white whitespace-nowrap">View Profile</span>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
      ...
      {/* Active Member Info */}
      <div className="mt-8 md:mt-16 container mx-auto px-6 md:px-12 flex flex-col items-center text-center">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <p className="text-[11px] md:text-xs tracking-[0.2em] text-gray-400 uppercase mb-4 md:mb-6">{activeMember.role}</p>
            <p className="max-w-xl mx-auto text-xs md:text-sm text-gray-300 font-light leading-relaxed italic">"{activeMember.bio}"</p>
          </motion.div>
        </AnimatePresence>
        
        <div className="mt-8 md:mt-12 flex items-center gap-4">
          <div className="text-xs md:text-sm font-serif tracking-widest">
            {String(activeIndex + 1).padStart(2, '0')} <span className="text-gray-600 mx-2">/</span> 06
          </div>
        </div>

        {/* Mobile controls */}
        <div className="flex md:hidden gap-6 mt-6 items-center">
          <button 
            onClick={() => paginate(-1)} 
            className="px-4 py-2 border border-white/20 rounded-full text-white hover:bg-white/10 active:bg-white/20 transition-colors uppercase text-[11px] tracking-widest flex items-center gap-2"
          >
